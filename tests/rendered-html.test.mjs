...
test("exports a CloudBase-ready static portfolio", async () => {
  ...
  assert.match(html, /全球化经营分析与决策驾驶舱/);
  assert.match(html, /经营分析模型 Demo/);
  assert.match(html, /目标达成与经营拆解/);
  ...
  assert.doesNotMatch(html, /游戏运营舆情风险预警与归因/);
...
test("ships portfolio assets and site metadata", async () => {
  ...
  assert.match(projectSection, /useState<ProjectFilter>\("business"\)/);
